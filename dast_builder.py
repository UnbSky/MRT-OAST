...
def build_sequence_dast(node, cache_dict):
    sequence = []
    sequence.append("Root")
    sequence.append("[")
    sequence.extend(get_sequence_dast(node, cache_dict))
    sequence.append("]")
    return sequence

def get_sequence_dast(node, cache_dict):
    token, children = get_token_ast(node, cache_dict), get_child_ast(node, cache_dict)
    sequence = []
    if token is not None and token is not "" and token not in ignore_nodes:
        ...
        #body
        for child in body:
            bchild_seqs = get_sequence_dast(child, cache_dict)
            if len(bchild_seqs) > 0:
                sequence.extend(bchild_seqs)
    return sequence
